...
public class UserDAO {
  ...
  public static void addUser(Connection conn, Scanner myObj) {
    ...
    if (rOrH.equals("r") || rOrH.equals("R")) {
      ...
      String cardNum = myObj.nextLine(); // Read user input // TODO must be int? 
  ...
  public static void rentersReviewListings(Connection conn, Scanner myObj){
    ...
    try {
      Statement stmt = conn.createStatement();
      String sql = "SELECT * FROM rentersReviewListings WHERE renterSIN = " + DAO.loggedInUser + " AND listID = " + listID + ";";
      ResultSet rs = stmt.executeQuery(sql);
      if (rs.next()) {
    ...
    try {
      Statement stmt = conn.createStatement();
      String sql = "SELECT * FROM Booked WHERE renterSIN = " + DAO.loggedInUser + " AND listID = " + listID + ";";
      ResultSet rs = stmt.executeQuery(sql);
      if (rs.next()) {
